...
def test():
    ...
    robot.set_dofs_position(torch.zeros(len(dofs_idx), device=gs.device), dofs_idx)

    for i in range(100):
        scene.step()
        pos = robot.get_pos()
        if torch.isnan(pos).any() or torch.max(torch.abs(pos)) > 100:
            print(f"Physics exploded at step {i}! Pos: {pos}")
            return
