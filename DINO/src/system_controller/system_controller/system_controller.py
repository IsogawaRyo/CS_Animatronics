...
class SystemController(Node):
    def __init__(self):
        ...
        self.controllerMap = os.path.join(dino_root, "ControllerMap.json")
        ...
        self.file_list = []
        self.cursor_index = 0
        self.record_dir = os.path.join(os.environ.get("DINO_ROOT_DIR", os.path.expanduser("~/CS_Animatronics/DINO")), "MotionFiles")
    ...
    def find_audio_directory(self):
        ...
        dino_root = os.environ.get("DINO_ROOT_DIR", os.path.expanduser("~/CS_Animatronics/DINO"))
        audio_dirs = [
            os.path.join(dino_root, "AudioFiles"),
            os.path.expanduser("~/CS_Animatronics/DINO/AudioFiles"),
            "/Users/isogawaryou/CS_Animatronics/DINO/AudioFiles"
        ]
    ...
    def AssignMotion(self):
        ...
        if int(selectedButton) in range(0,12):
            ...
            dino_root = os.environ.get("DINO_ROOT_DIR", os.path.expanduser("~/CS_Animatronics/DINO"))
            fileFounded = os.listdir(os.path.join(dino_root, "MotionFiles"))
            
            # Remove DS_Store if exists
            path = os.path.join(dino_root, "MotionFiles", fileSelected)
            if '.DS_Store' in fileFounded:
                fileFounded.remove('.DS_Store')
            file = open(os.path.join(dino_root, "ControllerMap.json"), "w")
