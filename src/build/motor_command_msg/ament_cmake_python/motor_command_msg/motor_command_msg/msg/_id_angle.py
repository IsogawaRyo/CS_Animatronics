...
class IdAngle(metaclass=Metaclass_IdAngle):
    ...
    def __init__(self, **kwargs):
        ...
        if 'ids' not in kwargs:
            self.ids = numpy.zeros(10, dtype=numpy.uint8)
        else:
            self.ids = numpy.array(kwargs.get('ids'), dtype=numpy.uint8)
            assert self.ids.shape == (10, )
        if 'angles' not in kwargs:
            self.angles = numpy.zeros(10, dtype=numpy.int32)
        else:
            self.angles = numpy.array(kwargs.get('angles'), dtype=numpy.int32)
            assert self.angles.shape == (10, )
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 10, \
                    "The 'ids' numpy.ndarray() must have a size of 10"
                self._ids = value
                return
            ...
            assert \
                ...
                "The 'ids' field must be a set or sequence with length 10 and each value of type 'int' and each unsigned integer in [0, 255]"
    ...
    def angles(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 10, \
                    "The 'angles' numpy.ndarray() must have a size of 10"
                self._angles = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ...
                "The 'angles' field must be a set or sequence with length 10 and each value of type 'int' and each integer in [-2147483648, 2147483647]"
